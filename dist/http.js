...
export async function apiFetch(path, options = {}) {
    ...
    try {
        ...
        throw serverError(`Network error: ${err instanceof Error ? err.message : String(err)}`);
    }
    ...
    if (resp.status === 401 && !noAuth) {
        ...
        try {
            ...
            throw serverError(`Network error on retry: ${err instanceof Error ? err.message : String(err)}`);
        }
        if (resp.status === 401) {
