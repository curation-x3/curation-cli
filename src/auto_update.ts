 ...
 */
export async function maybeAutoUpdate(currentVersion: string): Promise<void> {
  // Check env disable
  if (process.env.CURATION_AUTO_UPDATE === "0") return;
  ...
  if (cache?.checked_at) {
    ...
    if (elapsed < 24 * 60 * 60 * 1000 && cache.status === "current") return;
  }
